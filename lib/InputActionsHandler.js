...
class InputActionsHandler {
   ...
   * @param  {} actionType 'None', 'ShortClickToggle', 'StationarySwitchToggle',
   *                       'StationarySwitchOnOff', 'ShortClickToggleLongClickDim',
   *                       'DualShortClickOnOffLongClickDim', 'StationarySwitchSceneSelector'
   *                       'MomentarySwitchSceneSelector'
   * @param  {} group1     Group ID for primary scence (recalled with short-click or on-state)
   * @param  {} scene1     Scene ID for primary scence (recalled with short-click or on-state)
   * @param  {} group2     Group ID for secondary scence (recalled with long-click or off-state)
   * @param  {} scene2     Scene ID for secondary scence (recalled with long-click or off-state)
   */
  static builder(driverId, inputNumber, actionType, group1, scene1, group2, scene2) {
    let endpoint;
    const inputAndOptions = inputNumber - 1;
    const groupLSB1 = group1 % 256;
    const groupMSB1 = Math.round((group1 - groupLSB1) / 256);
    const groupLSB2 = group2 % 256;
    const groupMSB2 = Math.round((group2 - groupLSB2) / 256);
    ...
    // Single Momentary Switch (Push Button) as Toggle Switch
    ...
    // Single Stationary Switch as Toggle Switch
    if (actionType === 'StationarySwitchToggle') {
      return ([
        [inputAndOptions, 0x0d, endpoint, 0x06, 0x00, 0x02],
        [inputAndOptions, 0x03, endpoint, 0x06, 0x00, 0x02],
      ]);
    }

    // Single Stationary Switch as On/off Switch
    if (actionType === 'StationarySwitchOnOff') {
      return ([
        [inputAndOptions, 0x0d, endpoint, 0x06, 0x00, 0x01],
        [inputAndOptions, 0x03, endpoint, 0x06, 0x00, 0x00],
      ]);
    }

    // Single Momentary Switch (Push Button) as Dimmer Switch
    ...
    // Double Momentary Switch (Push Buttons) as Dimmer Switch
    // NOTE: This includes InputActions for both inputs!
    // Should only be used for inputNumber === 1
    if (actionType === 'DualShortClickOnOffLongClickDim') {
      if (inputNumber === 2) return ([[]]);
      return ([
        [0x00, 0x07, endpoint, 0x06, 0x00, 0x01],
        [0x00, 0x06, endpoint, 0x08, 0x00, 0x05, 0x00, 0x32],
        [0x00, 0x0b, endpoint, 0x08, 0x00, 0x07],
        [0x01, 0x07, endpoint, 0x06, 0x00, 0x00],
        [0x01, 0x06, endpoint, 0x08, 0x00, 0x05, 0x01, 0x32],
        [0x01, 0x0b, endpoint, 0x08, 0x00, 0x07],
      ]);
    }

    // Single Stationary Switch as Scene Selector Switch
    if (actionType === 'StationarySwitchSceneSelector') {
      return ([
        [inputAndOptions, 0x0d, endpoint, 0x05, 0x00, 0x05, groupLSB1, groupMSB1, scene1],
        [inputAndOptions, 0x03, endpoint, 0x05, 0x00, 0x05, groupLSB2, groupMSB2, scene2],
      ]);
    }

    // Single Momentary Switch (Push Button) as Scene Selector Switch
    if (actionType === 'MomentarySwitchSceneSelector') {
      return ([
        [inputAndOptions, 0x07, endpoint, 0x05, 0x00, 0x05, groupLSB1, groupMSB1, scene1],
        [inputAndOptions, 0x06, endpoint, 0x05, 0x00, 0x05, groupLSB2, groupMSB2, scene2],
      ]);
    }

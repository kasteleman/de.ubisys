...
class BoundClusterHandler {
  ...
  /**
   * Triggers the 'seton' flow cards.
   * @private
   */
  _setOnCommandHandler() {
    this.log('_setOnCommandHandler input =', this._input);

    this._device.triggerFlow({ id: `${this._input}seton` })
      .then(() => this.log('flow was triggered', `${this._input}seton`))
      .catch(err => this.error('Error: triggering flow', `${this._input}seton`, err));
  }

  /**
   * Triggers the 'setoff' flow cards.
   * @private
   */
  _setOffCommandHandler() {
    this.log('_setOffCommandHandler input =', this._input);

    this._device.triggerFlow({ id: `${this._input}setoff` })
      .then(() => this.log('flow was triggered', `${this._input}setoff`))
      .catch(err => this.error('Error: triggering flow', `${this._input}setoff`, err));
  }
